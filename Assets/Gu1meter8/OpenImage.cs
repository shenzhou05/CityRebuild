...
{
    //相机高度（俯视高度）
    const float CAMARAHEIGHT = 10.0f;

    void Start()
    {
        GameObject.Find("Main Camera").transform.rotation = Quaternion.AngleAxis(90, Vector3.right);
        GameObject.Find("Main Camera").transform.position = new Vector3(0.0f, CAMARAHEIGHT, 0.0f); ; 
    }
