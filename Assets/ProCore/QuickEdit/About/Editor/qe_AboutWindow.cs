 ...
 */
public class qe_AboutWindowSetup : AssetPostprocessor
{
#region Initialization
		...
		string[] movedFromAssetPaths)
	{
		...
		foreach(string str in entries)
			if( qe_AboutWindow.Init(str, false) )
				break;
	}
...
public class qe_AboutWindow : EditorWindow
{
...
#region User Settings
	...
	const string ABOUT_ROOT = "Assets/ProCore/QuickEdit/About";
...
#region Init
	...
	// 	if(entries.Length > 0)
	// 		qe_AboutWindow.Init(entries[0], true);
	// }
	...
	public static bool Init (string aboutEntryPath, bool fromMenu)
	{
		...
		if(fromMenu || EditorPrefs.GetString(identifier) != version)
		{
			...
			if(GetField(aboutEntryPath, "name: ", out tname))
				win = (qe_AboutWindow)EditorWindow.GetWindow(typeof(qe_AboutWindow), true, tname, true);
			else
				win = (qe_AboutWindow)EditorWindow.GetWindow(typeof(qe_AboutWindow));
